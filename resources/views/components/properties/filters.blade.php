...
@php
    ...
    if (! is_null($minSelected) && ! in_array($minSelected, $minOptions, true)) {
        $minOptions[] = $minSelected;
        sort($minOptions, SORT_NUMERIC);
    }

    $maxOptions = array_values(array_filter($maxRange, fn ($p) => is_null($minSelected) ? true : $p > $minSelected));
    if (! is_null($maxSelected) && $maxSelected !== $capValue && ! in_array($maxSelected, $maxOptions, true)) {
        $maxOptions[] = $maxSelected;
        sort($maxOptions, SORT_NUMERIC);
    }
@endphp
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-4">
            ...
            <flux:field class="col-span-1">
                ...
                <flux:select variant="listbox" wire:model.live="sort">
                    ...
                    <flux:select.option value="price_asc">
                        <div class="flex items-center gap-2">
                            <flux:icon.arrow-up variant="mini" class="text-zinc-400" />
                            Price
                        </div>
                    </flux:select.option>
                    <flux:select.option value="price_desc">
                        <div class="flex items-center gap-2">
                            <flux:icon.arrow-down variant="mini" class="text-zinc-400" />
                            Price
                        </div>
                    </flux:select.option>
                    <flux:select.option value="beds_asc">
                        <div class="flex items-center gap-2">
                            <flux:icon.arrow-up variant="mini" class="text-zinc-400" />
                            Beds
                        </div>
                    </flux:select.option>
                    <flux:select.option value="beds_desc">
                        <div class="flex items-center gap-2">
                            <flux:icon.arrow-down variant="mini" class="text-zinc-400" />
                            Beds
                        </div>
                ...
                <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex justify-stretch md:justify-end">
                        <x-flux::button
                            type="button"
                            variant="danger"
                            class="w-full transition-opacity disabled:cursor-not-allowed disabled:opacity-50 md:w-auto"
